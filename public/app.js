...
function airportLabel(a) {
  return `${a.code} | ${a.cityKo} ${a.nameKo} (${a.country === 'KR' ? t('korea') : t('japan')})`;
}
...
async function postJson(url, payload) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 400) {
      try { var errData = await res.json(); throw new Error(errData.error || t('err-input')); }
      catch(e) { if (e.message) throw e; throw new Error(t('err-input')); }
    }
...
function renderBudgetSummary() {
  ...
  wrap.innerHTML =
    ...
    '<div class="budget-rows">' +
      '<div class="budget-row"><span>✈️ 항공권</span><span>' + (flightCost ? fmt(flightCost) : t('not-selected')) + '</span></div>' +
      '<div class="budget-row"><span>🏨 숙소 (' + (selectedStay ? (selectedStay.nights || Math.max(1, days - 1)) + '박' : '-') + ')</span><span>' + (stayCost ? fmt(stayCost) : t('not-selected')) + '</span></div>' +
...
async function runPlan(extra = {}, syncAux = false) {
  ...
  if (destSourceNote) {
    ...
    var destSrcLabel = String(src).includes('google') ? t('source-ai-google') : isFallback ? '📋 규칙기반 추천 (폴백)' : t('source-ai-rec');
...
async function calculateDayRouteCost(dayNum) {
  ...
  try {
    ...
    for (var i = 0; i < data.segments.length; i++) {
      ...
      var fareText = seg.fareJPY > 0 ? '¥' + seg.fareJPY.toLocaleString() + ' (~' + seg.fareKRW.toLocaleString() + '원)' : t('free-label');
    ...
    var routeSourceLabel = data.source === 'ai' ? t('source-ai-calc') + routeModelInfo : data.source === 'distance_estimate' ? '📏 거리 기반 추정 (AI 미응답 시 자동 폴백)' : data.source === 'directions_api' ? '🗺 Google 경로 API' : 'ℹ️ 추정치';
...
document.addEventListener('click', function(e) {
  ...
  if (tgt.id === 'btnShareLink') {
    ...
    if (navigator.share) {
      navigator.share({ title: t('share-title'), text: st });
...
(async function showExchangeRate() {
  ...
  try {
    ...
    chip.textContent = t('fx-fallback');
  }
})();
    ...
    document.getElementById('saveConfirmBtn').addEventListener('click', async function() {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          showMemoToast(result.error || t('save-fail'));
        }
      } catch(e) {
...
async function loadMyPlansList() {
  ...
  try {
    ...
    var data = await resp.json();
    if (!resp.ok) {
      container.innerHTML = '<p class="my-plans-empty">' + (data.error || t('err-prefix')) + '</p>';
      return;
    }
...
async function loadPlanFromServer(planId) {
  try {
    ...
    if (!resp.ok || !data.plan) {
      showMemoToast(data.error || t('load-fail'));
      return;
    }
...
var I18N = {
  ...
  en: {
    ...
    'budget-note': 'Per person · ',
    'budget-note2': ' days · Food/transport/activities estimated',
    ...
    'share-title': 'JapanTravel Itinerary',
    'fx-loading': '¥/₩ Loading...',
    'fx-fallback': '100¥≈950KRW',
    ...
    'source-dist-est': '📏 Distance Estimate (AI fallback)',
    'source-google-route': '🗺 Google Routes API',
  ...
  ja: {
    ...
    'share-title': 'JapanTravel 여행 일정',
    'fx-loading': '¥/₩ 로딩...',
    'fx-fallback': '100¥≈950원',
    'provider-naver': '네이버',
    'provider-kakao': '카카오',
    'btn-save-plan': '💾 저장',
    'btn-my-plans': '📂 내 일정',
    ...
    'source-dist-est': '📏 거리 기반 추정 (AI 미응답 시 자동 폴백)',
    'source-google-route': '🗺 Google 경로 API',
