...
| Method | Path | 설명 |
|--------|------|------|
| POST | `/api/flights` | 항공권 탐색 (Amadeus 실데이터 or Mock) |
| POST | `/api/stays` | 숙소 탐색 (Amadeus 실데이터 or Mock) |
...
| 팩터 | 가중치 | 계산 방식 |
|------|--------|-----------|
| ratingScore | **0.23** | `rating / 5` (0~1) |
| reviewScore | **0.17** | `log(1 + reviewCount) / log(1 + 5000)` (0~1) |
| recentnessScore | **0.14** | `0.35 + reviewScore * 0.65` (0~1) |
| preferenceScore | **0.21** | `categoryFit(theme, primaryType)` - 테마 적합도 |
| budgetScore | **0.08** | `budgetFit(budget, primaryType)` - 예산 적합도 |
| mobilityScore | **0.17** | `1 - (centerDistKm / 20)` (0.1~1) - 도심 근접도 |

**최종 점수**:
```
composite = (rating*0.23 + review*0.17 + recentness*0.14
           + preference*0.21 + budget*0.08 + mobility*0.17) * 100
finalScore = round(clamp(composite - congestionPenalty * 30, 0, 100))
```
